                    ...
                    if event_obj["command_args"].startswith("="):
                        ...
                        if self.db_connection:
                            ...
                            try:
                                ...
                                self.db_connection.commit()
                                await self.message(channel, f"Info item 'phreak' added successfully with ID {item_id}.")
                                self.logger.info(f"Successfully added info item with ID {item_id}")
                                return
                            except Exception as e:
                                self.logger.error(f"Error adding info item: {e}")
                                await self.message(channel, f"Error adding info item: {e}")
                                self.db_connection.rollback()
                            finally:
                                cur.close()
                        ...
                        if self.db_connection:
                            ...
                            try:
                                ...
                                if not items:
                                    ...
                                    await self.message(channel, f"Info for 'phreak' ({len(items)} entries):")
                                    for item_id, value, username, timestamp in items:
                                        await self.message(channel, f"• [{item_id}] {value} (added by {username} on {timestamp.strftime('%Y-%m-%d')})")
                                return
                            except Exception as e:
                                self.logger.error(f"Error retrieving info item: {e}")
                                await self.message(channel, f"Error retrieving info item: {e}")
                            finally:
                                cur.close()
