...
def run(bot, event):
    ...
    bot.logger.info(f"TestMinus event: {event}")
    bot.logger.info(f"TestMinus trigger: {event['trigger']}")
    bot.logger.info(f"TestMinus command: {event.get('command', 'No command')}")
